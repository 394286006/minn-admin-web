...
class PropertiesResourceReader {
  ...
  static getInstance(pkg,locales,prefix,callback){
    return new PropertiesResourceReader(pkg,locales,prefix,callback);
  }
  ...
  ajaxLoad(pkg,locale,prefix){
    $.ajax({ url: pkg+'/'+locale.locale+'/'+prefix+'.properties' })
          ...
          .fail(jqXhr => {
            console.log('url load fail:'+pkg+'/'+locale.locale+'/'+prefix+'.properties'+',message:'+jqXhr.responseJSON.message);
          });
